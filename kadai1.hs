...
{-
(レイヤー名)Forward:入力された配列に対して、順方向の処理をして出力を返します
(レイヤー名)Backward:逆伝播してきた配列などを受け取り、逆方向の処理をして出力を返します
-}
...
sigmoidLayerBackward outputs backouts 
  = mul backouts (mul outputs (matmap (\x->alpha*(1-x)) outputs))
...
answer a b = if a'||b' then 1 else 0
  where
    a'=(a==1)
    b'=(b==1)
...
train ws log
  ...
  |otherwise = train wsn log'
    where
      ...
      error = closeEntropyErrorForward x2 t
      log' = log++[(sum (concat error))/4]
      
      b1 = closeEntropyErrorBackward x2 t
      b2 = sigmoidLayerBackward x2 b1
      (b3w,b3x) = affineLayerBackward input' b2 (ws!!0) 
      ws0' = matmap2  (\a b -> a-eps*b) (ws!!0) b3w
